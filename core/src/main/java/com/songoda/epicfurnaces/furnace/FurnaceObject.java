...
public class FurnaceObject {
    ...
    public FurnaceObject(Location location, Level level, String nickname, int uses, int toLevel, List<String> accessList, UUID placedBy) {
    ...
    public FurnaceObject(Block block, Level level, String nickname, int uses, int toLevel, List<String> accessList, UUID placedBy) {
    ...
    public void plus(FurnaceSmeltEvent e) {
        try {
            ...
            if (block.getType() != Material.FURNACE && block.getType() != instance.getBukkitEnums().getBurningFurnace()) {
                return;
            }
...
//            e.setResult(new ItemStack(e.getResult().getType(), r));
    ...
    private void syncName() {
        if (location.getBlock().getType() != Material.FURNACE && location.getBlock().getType() != instance.getBukkitEnums().getBurningFurnace()) {
            return;
        }

        instance.getCraftBukkitHook().setName(location.getBlock(), Methods.formatName(level.getLevel(), uses, false));
    }

    public void updateCook() {
        try {
            Block block = location.getBlock();
            if (block == null || (block.getType() != Material.FURNACE && block.getType() != instance.getBukkitEnums().getBurningFurnace())) {
                return;
            }
